...
<template>
    <section class="mb-4 h-[40px] flex">
        ...
        <div v-if="store.searchedPokemon" class="flex grow justify-end">
            <button v-if="!store.myPokemon.includes(userInput[0].toUpperCase() + userInput.slice(1).toLowerCase()) && userInput.toLowerCase() === store.searchedPokemon.name" class="bg-slate-300 border-2 border-red-950 px-5" @click="$emit('catchPokemon')">Catch!</button>
            <button v-else-if="store.myPokemon.includes(userInput[0].toUpperCase() + userInput.slice(1).toLowerCase()) && userInput.toLowerCase() === store.searchedPokemon.name" class="bg-slate-300 border-2 border-red-950 px-5" @click="$emit('releasePokemon')">Release!</button>
        </div>
    </section>
</template>